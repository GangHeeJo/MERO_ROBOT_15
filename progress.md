...
## 2026-07-24 작업 내역 — 본선 당일

> **본선 결과: 태극기 스토리지 도달 4초 전에 시간 종료로 못 들어감.** GO_TO_STORAGE 로직을 실전 피드백 받아가며 여러 번 재설계, 집기 실패 시 팔 안 올리는 버그 수정, POST_GRIP_SCAN 후진 순서 버그 수정, 새로 학습한 통합 가중치(`best_final.pt`) 테스트용 브랜치 분리 운영.

### 브랜치 전략 — `snapshot-1504`(안정) / `test-best-final`·`test-best-final-2`(가중치 실험)

- 대회 당일 실시간으로 코드를 계속 튜닝해야 했는데, 새로 학습한 가중치(`best_final.pt`)를 바로 주 브랜치에 넣기엔 검증이 안 된 상태라 리스크가 큼 → **`snapshot-1504`는 기존 `best.engine`(검증된 가중치) 그대로 안정 상태 유지**, 새 가중치는 `snapshot-1504`에서 딴 별도 브랜치(`test-best-final`)에서만 테스트
- 로직 개선(버그 수정, 튜닝)은 전부 `snapshot-1504`에 먼저 커밋하고, 그 커밋을 `test-best-final`/`test-best-final-2`로 매번 merge — 세 브랜치가 "가중치 파일 + `MODEL_PATH` + `trt_export.py --name` 옵션" 딱 그 차이만 나고 로직은 항상 동일하게 유지되도록 관리
- 두 번째로 재학습한 가중치는 `snapshot-1504`에서 새로 딴 `test-best-final-2`에 배치(첫 번째 `test-best-final`은 그대로 보존) — **본선에서 실제로 사용한 브랜치는 `test-best-final-2`**
- 중간에 젯슨 로컬 브랜치 포인터가 다른 브랜치를 가리키게 꼬이는 사고가 두 번 발생(`git checkout -B` 계열 명령 등으로 추정) — `git log --oneline`으로 실제 커밋 내역 확인 후 `git reset --hard origin/<branch>`로 복구. **브랜치 전환은 항상 `git checkout <이름>`만 쓰고, 다른 브랜치 커밋을 지금 브랜치에 pull 하려 하면 diverge 에러 나니 먼저 checkout부터 확인할 것**

### GO_TO_STORAGE — 하루 동안 여러 차례 재설계

실전 테스트하면서 매번 "이건 이래서 별로다"는 피드백을 받아 아래 순서로 계속 갈아엎음:

1. 태극기 안 보일 때 물체 밀집 방향 이동 + 정지대기(1초) + 좌우 정렬 → 정렬 끝나면 직진 접근(순차) 복원
2. 태극기 1프레임이라도 감지되면 정렬/접근 없이 즉시 정지하는 극단적 단순화판도 잠깐 시도했다가, "물체 집을 때처럼 중앙 정렬하고 전진하는 것"을 다시 요청받아 원복
3. **진입 직후 딱 한 번, 제자리에서 천천히 한 바퀴(`storage_initial_spin`) 먼저 훑어보고** 못 찾으면 기존 밀집 이동/회전 탐색으로 전환 — 그 자리에서 바로 보일 가능성을 놓치지 않기 위함
4. 정렬(회전)이 끝날 때까지 정지해있다가 그제서야 직진하던 방식이 "너무 굼뜨다"는 피드백 → **정렬(회전)과 접근(직진)을 매 프레임 동시에 섞어서 조향**하는 방식으로 재변경(물체 밀집 탐색과 비슷한 구조지만, "멀리서 많이 틀어지면 순수 회전만" 하는 로직은 없고 태극기 쪽은 항상 전진을 깔고 감)
5. `FLAG_AREA_SLOW_THRESHOLD`(감속 시작 면적)가 도달 판정 임계값의 15%밖에 안 돼서 접근 거리 대부분을 감속 속도로 기어갔던 버그 발견 → 70%로 상향했다가, 최종적으로는 **감속 로직 자체를 제거**하고 끝까지 `FLAG_APPROACH_SPEED` 고정 유지로 정리
6. 도달 판정 후 마지막 직진 버스트(`FLAG_FINAL_FORWARD_SECS`) 0.5 → 0.25 → 0.15초로 계속 단축, 끝나면 `flag_arrived` 플래그 거쳐 다음 프레임에 종료 체크하던 것도 **그 자리에서 바로 `break`로 즉시 프로그램 종료**하도록 정리(불필요한 한 프레임 지연 제거)
7. 태극기 정렬/접근 중 순간적으로 놓쳐도 즉시 재탐색하지 않고 `FLAG_MISS_GRACE_FRAMES`(10)까지는 정지 대기하는 grace 로직 추가(물체 집기의 `TARGET_MISS_GRACE_FRAMES`와 동일 패턴)
8. 회전 속도들(`FLAG_INITIAL_SPIN_SPEED`/`FLAG_SEARCH_SPEED`) 0.05~0.07 → 0.15로 상향(체감상 너무 느렸음)

**GO_TO_STORAGE 진입 조건도 변경**: 기존엔 `PICK_PHASE_SECS`(2분30초) 시간 경과만 트리거였는데, `grip_success_count`(실제 성공한 집기 횟수, 아래 버그 수정 이후로 신뢰 가능해진 값) **7개 도달 시에도 시간 안 기다리고 바로 전환**되도록 추가(둘 중 하나만 만족해도 전환). 재시작 시 이 카운트가 리셋되는 문제 대응으로, 남은 경기 시간처럼 "이미 집은 개수"도 시작할 때 직접 입력받게 함.

**돌진 직전 최종 클래스 확인 기능 추가**(물체 집기 쪽): `select_target()`이 한 프레임 오인식(예: d8을 d6로 잘못 봄)으로 엉뚱한 물체에 락 걸리면 클래스 재검증 없이 그대로 그립까지 가던 문제 대응 — 정밀 정렬 중 매 프레임 (시각, 클래스)를 기록해뒀다가, 직진 접근(돌진) 진입 직전에 최근 500ms 동안의 최빈 클래스를 락 시점 클래스와 비교, 다르면 락 해제하고 재탐색으로 복귀.

### GRIPPING/POST_GRIP_SCAN 버그 두 건 발견 및 수정

1. **집기 실패해도 팔이 그대로 올라가던 버그**(`robot.ino`): `gripperClose()`의 반환값(성공/실패)을 저장도 안 하고 버린 채 무조건 `LIFTING`으로 진행하고 있었음. 게다가 `{"status":"grip_failed"}`를 실제로 보내는 코드가 어디에도 없어서(문서에만 있던 상태) Jetson 쪽 실패 처리 분기가 지금까지 한 번도 발동한 적이 없었음. 반환값 확인해서 실패 시 팔 안 올리고 바로 `grip_failed` 전송 + `IDLE` 복귀하도록 수정. 이제 `"gripped"` 수신 = 진짜 성공한 픽만 오는 게 보장돼서, 이 신호로 `grip_success_count`(위 GO_TO_STORAGE 전환 조건에 사용)를 정확히 셀 수 있게 됨
2. **POST_GRIP_SCAN에서 후진을 건너뛰던 버그**: `if target:` 체크가 후진(`POST_GRIP_BACKUP_SECS`) 체크보다 코드상 먼저 있어서, 집자마자 근처에 다른 물체가 바로 보이면 후진을 한 번도 안 하고 바로 `SEARCHING`으로 넘어가버림 — 실전 테스트 중 "집은 다음에 뒤로 안 간다"는 리포트로 발견. 후진 구간이 무조건 먼저 끝나야 `target` 체크가 동작하도록 순서 변경

### 새 가중치 학습 — `best_final.pt` (도형+과일+flag 전 클래스 통합, 2회 재학습)

- `train.ipynb`를 Roboflow API 병합 방식에서 **zip 파일 하나(전 클래스 포함) 업로드** 방식으로 단순화, 결과 파일명이 `best_final.pt`로 나오도록 run 이름 지정
- 1차 학습 결과 클래스 이름에 `1_d6`, `2_d8`처럼 **순번 접두어가 그대로 붙어나옴** — `main.py`의 `SHAPE_CLASSES`/`FRUIT_CLASSES` 매칭이 정확한 문자열 기준이라 이대로면 인식 자체가 안 됨. `model.names`를 로드 직후 in-place로 수정해도 실제 `track()` 추론 결과엔 반영이 안 되는 걸 확인(ultralytics가 내부적으로 체크포인트에서 이름을 별도로 다시 읽어오는 것으로 추정) → 별도의 `CLASS_NAMES` 매핑 테이블을 만들어서 탐지 루프에서 그걸 쓰도록 우회
- 2차 재학습 시 데이터셋 쪽에서 접두어 제거하는 `data.yaml` 후처리 셀을 학습 전에 실행해서, 이번엔 모델에 클래스 이름이 접두어 없이 깨끗하게 박힘(`d6, d8, ..., flag, unknown` 확인) → `test-best-final-2`는 `CLASS_NAMES` 우회 코드 없이 `snapshot-1504`와 코드 완전히 동일, 가중치 파일만 다르게 유지
- `trt_export.py`에 `--name` 옵션 추가해서 `best.pt`가 아닌 커스텀 이름 가중치(`best_final.pt`)도 TensorRT 변환 가능하게 함(`best_final.engine`)

### 기타 버그/튜닝

- 그리퍼 열기 명령과 직진 접근 시작이 완전히 동시였던 것 → `GRIPPER_OPEN_LEAD_SECS`(0.3초, 그리퍼 물리 개방 시간과 맞춤) 대기 후 직진하도록 수정
- `ARM_SPEED`/`ARM_DOWN_RAW`/`GRIPPER_SPEED` 실전 테스트하며 여러 차례 재조정(최종: `ARM_SPEED=110`, `ARM_DOWN_RAW=1620`, `GRIPPER_SPEED=200`)
- `PRECISE_ALIGN_TURN_SPEED`(정밀 정렬 회전 속도) 0.25 → 0.18 (타겟 잡은 후 회전이 너무 빠르다는 피드백), `TARGET_MISS_GRACE_FRAMES` 10 → 3
- `select_target()` 오른쪽(cx) 우선 → area(면적 큰 것=보통 더 가까운 것) 우선으로 재변경(오른쪽 우선이 더 가깝고 큰 물체 무시하고 먼 쪽으로 이동하는 비효율 있었음)
- 경기 타이머를 `--timer` 플래그 없이 항상 화면에 표시하도록 변경
- KY-032 적외선 물체 통과 카운터를 `main` 브랜치에서 이식했다가(오탐 이력 있어서 관측용으로만 사용), 그립 성공 카운트가 훨씬 신뢰도 높다고 판단해서 **최종적으로 전부 제거**
- `wall_line_test.py`(바닥-벽 경계선 감지, Hough 직선 변환 기반) 실험 — 벽 감지로 뭔가 활용해보려 했으나 이번 대회 로직에는 최종 반영 안 함

### 다음 세션 우선순위

1. `FLAG_AREA_STOP_THRESHOLD`(20만), `FLAG_FINAL_FORWARD_SECS`(0.15초), `storage_initial_spin` 관련 값들 전부 실측 안 된 추정치 — 필드에서 실측하며 재조정 필요
2. 4초 부족했던 원인 분석(탐색에 시간 다 써서 그런 건지, 정렬/접근 자체가 느려서인지) — `_wheel_log` CSV/영상 있으면 리뷰
3. UGV 배터리 순간 전압 드롭 원인 — 여전히 미해결(배터리 스왑 테스트 못 함)
4. `test-best-final`/`test-best-final-2` 검증 끝나면 최종적으로 어느 가중치를 `snapshot-1504`(또는 `main`)에 반영할지 결정

---

## 2026-07-23 작업 내역

> 카메라 hang/재연결 완전 정복, UGV 배터리 순간전압 드롭 원인 조사(미해결), 경기 시계 동기화, GO_TO_STORAGE 대대적 재설계(DROPPING 삭제 + 밀집 이동 탐색), TensorRT 전환+FPS 병목 실측, 신규 과일 데이터셋 Roboflow 병합 워크플로우 정리.

### 카메라 캡처 hang/재연결 — 근본 원인 발견 및 3단계 방어 구축

**증상**: `select() timeout` 뜨면서 카메라가 몇 초~영구적으로 죽고, 이 순간 바퀴도 같이 멈춰서 "갑자기 전원 나간 것처럼" 보였음.

**진짜 원인**: 메인 루프에서 `cap.read()`를 직접 호출하고 있었음(주석엔 "별도 스레드로 분리"라고 돼있었지만 실제 구현이 안 돼 있었던 오래된 버그). `select() timeout`으로 `cap.read()`가 몇 초 블로킹되면 그 사이 `control_wheels()`/`send_idle()`이 전혀 안 불려서 ESP32의 하트비트 워치독(3초 무명령시 자동 정지, `progress.md` 기존 기록)이 발동 — 배터리 전압 문제가 아니라 카메라 블로킹→명령 끊김→ESP32 자체 안전정지였음.

**고친 것 (`vision/src/main.py`)**:
1. **캡처를 진짜 별도 스레드로 분리** — 메인 루프는 `_cap_latest_frame`(공유 상태, lock 보호)만 읽고, 카메라가 멈춰도 마지막 프레임으로 계속 돌면서 바퀴 명령을 끊김없이 보냄
2. **1단계 — 빠른 재연결**: `read()`가 False를 20회 연속 반환하면 `release()` 후 재오픈(release 직후 바로 열면 "device busy"로 실패해서 1초 텀 둠)
3. **2단계 — 좀비 USB 대응**: 재오픈 자체가 "index out of range"류로 실패하면 `USBDEVFS_RESET` ioctl로 그 USB 장치만 소프트 리셋(케이블 재연결과 비슷한 효과) 후 재시도. sudo 권한 필요 — udev 규칙(`/etc/udev/rules.d/99-arducam-usbreset.rules`, ArduCAM `04b4:4950`에 `MODE="0666"`) 추가해서 sudo 없이도 되게 함
4. **3단계 — 완전 hang 대응 워치독**: `read()` 호출 자체가 영원히 안 돌아오는 경우(1·2단계가 발동할 기회도 없음)를 별도 워치독 스레드가 "마지막 프레임 받은 지 5초 초과"로 감지 → 죽은 캡처 스레드는 버려두고(daemon이라 프로세스 종료시 정리됨) 완전히 새 카메라 객체+새 캡처 스레드를 통째로 새로 띄움
5. **재연결 시 인덱스 재검색**: 고정된 `CAMERA_INDEX_OBJ` 대신 매 시도마다 이름(`arducam`)으로 다시 검색 — USB 재연결 시 `/dev/videoN` 번호가 바뀌는 경우 대응
6. **오픈 백엔드 명시**: `cv2.VideoCapture(index)`처럼 정수만 주면 V4L2 실패시 엉뚱한 백엔드(obsensor 등)로 넘어가며 헷갈리는 에러(`index out of range`)를 냄 — `cv2.VideoCapture(f"/dev/video{index}", cv2.CAP_V4L2)`로 백엔드 직접 명시해서 회피

**실전 로그로 확인된 부작용**: 카메라 hang 중에도 정밀 정렬 로직이 멈춘 화면 기준으로 계속 조향 명령을 반복 전송 → hang 풀리고 새 프레임 오면 bytetrack이 다른 물체로 인식(추적 ID 불연속) → "타겟 놓침 → 재탐색"으로 빠짐. 즉 카메라 하나가 몇 초 멈추면 그 시간만큼 정렬 진행이 통째로 날아감 — 근본적으로는 hang 자체를 줄이는 게 답이지만(USB 대역폭 이슈라 완전 제거는 어려움), 재연결 체계 덕에 최소한 프로그램이 죽거나 바퀴가 안전정지하는 일은 없어짐.

### UGV 배터리 순간 전압 드롭 — 조사했지만 미해결

**증상**: 바퀴가 갑자기 반응 없어짐. 이때 화면/로그는 계속 정상(메인 루프 안 멈춤), 그리퍼/팔(별도 배터리)도 정상. 배터리 전압이 순간 0.5V 이하로 표시된 적 있음. 충전기 연결하면 즉시 12V로 회복.

**배제한 것**:
- 카메라 문제 아님 (위 스레드 분리 이후에도 "화면 정상, 바퀴만 안 됨" 증상 재확인됨)
- 시리얼(USB) 연결 끊김 아님 (`dmesg`에 ESP32/OpenRB USB disconnect 기록 전무)
- 물리적 커넥터 접촉불량 아님 (여러 차례 확인, 사용자가 배제)
- 완전 방전 문제만은 아님 (완충 후 셀 3개 전압 확인 — 다 4V+, 밸런스 정상)

**현재 가설**: 배터리(또는 BMS)의 순간 최대 방전전류(C-rate) 한계가 UGV 6륜 동시 구동 요구 전류를 못 따라가서 전압이 순간적으로 크게 처지는 것으로 추정. 완충 상태에서도 발생 가능 — 충전량이 아니라 "그 순간 뽑아낼 수 있는 전류" 자체의 한계 문제로 보임. 충전기 연결시 즉시 회복된 건 충전기가 부하 전류 일부를 대신 부담해서 배터리 자체 부담이 줄었기 때문(V = V_open - I×R_internal).

**소프트웨어로 조치한 것 (근본 해결은 아님, 완화만)**:
- 바퀴 속도 램핑 추가 — `control_wheels()` 최종 L/R을 초당 `MAX_ACCEL_PER_SEC`(1.0, 임의값)만큼씩만 변하게 제한, 급가속으로 인한 순간 전류 스파이크 완화. 단 "짧은 순간 스파이크"에만 효과 있고 "지속적으로 높은 전류가 필요한 상황"(예: 지속 고속 주행)엔 한계 있음을 인지 중
- ESP32/OpenRB 시리얼 write 실패시 자동 재연결 로직 추가 (카메라와 동일 패턴, `/dev/serial/by-id` 기반이라 포트 번호 바뀌어도 안전) — 단, 이번 증상은 통신 자체가 안 끊겼던 것으로 확인돼서 이 증상의 직접적인 해결책은 아니었음

**남은 것 (미검증, 다음 세션 우선순위)**:
- **배터리 스왑 테스트**: 남는 예비 배터리(같은 스펙)로 교체해서 같은 부하 재현했을 때도 전압이 떨어지는지 확인 필요. 이게 원인 판별의 핵심인데 시간이 없어서 못 함
  - 스왑해도 동일 증상 → 배터리 개체 문제 아니라 이 로봇 자체 전류 요구량이 이 스펙 배터리로는 구조적으로 부족한 것(더 높은 C-rate 배터리 필요)
  - 스왑하니 괜찮음 → 원래 배터리 개체(셀 노후화 등) 문제
- 배터리→모터 배선 굵기(전선 저항) 육안 확인 — 접촉불량 아니어도 전선 자체가 가늘면 전압 강하 커짐
- 급회전+고속 동시 요구 상황을 줄이는 로직(조향각 크면 속도 자동 캡) 검토 안 함
- 배터리 병렬 연결(역류방지 다이오드 필요) 또는 더 높은 C-rate 배터리 교체 — 하드웨어팀 판단 필요

### 경기 시계 동기화 — 남은 시간 입력 기능 추가

**문제**: `match_start_time`이 "실제 경기 시작 시각"이 아니라 "프로세스가 그 줄에 도달한 시각"이라, 경기 도중(예: 카메라 hang으로) `main.py`를 재시작해야 하면 `PICK_PHASE_SECS`(150초) 카운트다운이 재시작 시점부터 새로 시작돼서 실제 경기 종료 전에 `GO_TO_STORAGE` 전환을 영영 못 하는 문제가 있었음.

**고친 것**: 클래스 선택 다음에 "남은 경기 시간"을 MM:SS로 입력받아 `match_start_time`을 그만큼 과거로 당겨서 보정. 처음 시작이면 그냥 Enter = 3:00(전체 경기시간)으로 기존과 동일 동작. `--storage-only` 모드는 컴포넌트 테스트라 이 프롬프트 스킵.

### GO_TO_STORAGE 대대적 재설계

- **DROPPING 상태 완전 삭제**: 모빌리티 자체가 스토리지 안까지 물리적으로 진입하는 방식으로 규칙이 바뀌어서, "도달 시 정지 + dump 명령 전송 + 컨테이너 열림 대기" 전체 흐름 제거. `send_dump()`, `openrb_dumped`, `DROP_TIMEOUT_SECS` 등 전부 삭제
- **태극기 좌우(cx) 정렬**: 보이는 모든 태극기 박스의 cx **단순 평균**(가중치 없음)을 화면 중앙 세로선에 맞추는 좌우 정렬만 수행(상하 정렬 없음 — 거리는 area로 판단). 여러 태극기 있어도 평균으로 안정적으로 방향 잡음(기존엔 confidence 최고 1개만 선택해서 흔들릴 수 있었음)
- 회전 방향이 반대로 되어있던 버그 팀원이 수정 (`a6a40ac`)
- 정렬+접근을 순차(먼저 정렬, 그다음 직진)가 아니라 **회전+전후진 동시 조향**으로 변경(팀원 작업, `4667d35`)
- **물체 탐색 중 밀집 방향 이동 임계값 상향**: `MIN_DETECTED_FOR_EXPLORE` 2→3(팀원, `e45c7be`)→3(재확인)
- **물체 1개 이하일 때 전진 절대 금지**: 안 보이는 방향으로 무작정 전진하면 벽에 부딪힐 수 있어서, 못 찾으면 제자리 회전만 계속(전 버전은 1초 회전→2초 강제직진 반복이었음, 그 강제직진 로직 삭제)
- **경기 시작 오프닝 무브 추가**: SEARCHING 진입 직후 `OPENING_ARC_SECS`(3초)간 전진하며 좌회전하는 코너링 방식으로 필드 중앙 쪽 진입 (직진→회전 순차가 아니라 동시)
- **태극기 탐색 중 물체 밀집 방향 이동 신규 추가**: `storage_phase==0`(태극기 안 보일 때)에서 기존엔 제자리 회전만 했는데, 물체가 `STORAGE_EXPLORE_MIN_DETECTED`(6개) 이상 보이면 SEARCHING과 동일한 밀집도 가중 로직으로 그 방향으로 회전+전진 동시 이동하도록 추가. **근거**: 필드 배치도 확인 결과 물체는 42포인트 격자로 필드 상단 대부분을 채우고, storage/start point는 하단 양쪽 구석의 물체 없는 빈 통로에 위치 — 격자 구석(뒤쪽)에 있으면 하단 통로 시야가 가려서 순수 회전으로는 태극기를 못 볼 수 있는데, 물체 밀집 방향(=격자 무게중심)으로 수렴하면 격자 중앙 쪽으로 이동하게 되고, 거기서는 하단 통로까지 시야가 덜 가려서 회전 탐색 성공률이 올라감. SEARCHING(임계값 3)보다 임계값을 높여(6) 신중하게 이동, 속도(`STORAGE_EXPLORE_SPEED=0.35`)는 SEARCHING의 `MOVE_SPEED`(0.25)보다 높게. 태극기 한 번이라도 보이면 이 로직 무시하고 즉시 정렬 단계로 우선 전환(기존 동작 그대로)
- **`--storage-only` 플래그 추가**: SEARCHING/GRIPPING 건너뛰고 시작하자마자 GO_TO_STORAGE 진입해서 태극기 정렬+접근 로직만 단독 테스트 가능
- 태극기 area 20만 도달 시 0.5초 직진 후 정지 로직 추가(팀원, `22babaa`) — 완전 무정차 진입은 위험 부담 있어서 최종 근접 시엔 정지하는 걸로 재조정된 것으로 보임(DROPPING 삭제 방향성과는 약간 다른 절충안, 확인 필요)
- `FLAG_APPROACH_SPEED` 0.2 → 0.3 → (팀원이) 0.4로 상향, `FINAL_APPROACH_SPEED`(grip 직전 직진) 0.15 → 0.2 상향

### TensorRT 전환 + FPS 병목 실측

- `best.engine`으로 전환 완료(2026-07-22, FP16, 변환 523.7s)
- **중요 발견**: TensorRT 전환 자체는 FPS에 거의 영향 없었음(PyTorch/엔진 둘 다 ~22fps로 동일) — 진짜 병목은 **`sudo jetson_clocks` 안 돌려서 GPU 클럭이 동적으로 낮아져 있던 것**이었음. `jetson_clocks` 실행 후 추론 34ms→20ms, fps 22→36.5로 대폭 개선. `nvpmodel -q`로 MAXN_SUPER 확인하는 것만으론 부족 — 그건 "허용 최대 클럭"만 정하고, 실제 클럭은 부하 낮을 때 동적으로 내려가서 `jetson_clocks`로 강제 고정해야 함. **재부팅하면 초기화되므로 젯슨 켤 때마다 매번 다시 실행 필요** (`progress.md` STEP 5.5로 이미 반영돼 있음)

### 신규 테스트/디버깅 스크립트

- `camera_test.py`: 카메라만 단독 가동(YOLO 없음), 브라우저 :8082 스트림 — CLAUDE.md에 문서화만 되어있고 실제 파일은 없었던 것 신규 작성
- `yolo_cam_test.py`: 카메라+YOLO(`best.engine`) 탐지 테스트, 시리얼 연결 없어 로봇 안 움직임, 브라우저 :8083 — 기존 `flag_test.py`(best.pt 하드코딩, flag만 필터) 대체. `--pt`(PyTorch 비교용), `--resize`(전처리 리사이즈 병목 확인용) 플래그, 박스 면적(area) 오버레이 표시, 구간별(read/resize/model) 타이밍 로그 추가
- `main.py --record`/`--record-raw`: 테스트 중 프레임을 5fps로 샘플링해 JPEG 저장(백그라운드 스레드, 부하 최소화). `--record-raw`는 박스 없는 원본 프레임 저장
- `vision/logs/<시각>_wheel.csv`: `control_wheels()` 매 호출마다 timestamp/elapsed/state/L/R/battery_v 기록 — 배터리 전압 이슈 진단용으로 항상 켜짐

### 데이터셋 — 신규 과일 사진 Roboflow 병합 워크플로우

- 실제 예선에 쓰인 과일 사진을 로컬 `vision/DATASET/image-based/`에 추가함 — **로컬 폴더에 넣는 것과 Roboflow 프로젝트에 업로드/라벨링하는 건 별개**라는 점 확인(자동 동기화 안 됨)
- Roboflow 프로젝트: workspace `s-workspace-qdwfc`, project `merong-gurme` (표시 이름 "MERONG"과 실제 URL 슬러그가 다름 — 헷갈리기 쉬움, 프로젝트 URL에서 직접 확인 필요)
- 정다면체+과일(Roboflow API 스니펫 다운로드) + flag(zip 업로드) **두 데이터셋을 하나로 병합**하는 스크립트 작성 — 각 데이터셋의 `data.yaml`에서 클래스 목록 읽어와 flag 클래스 인덱스를 base 클래스 개수만큼 offset해서 라벨 재작성, 이미지/라벨 파일명 충돌 방지(`base_`/`flag_` 접두사), 병합 결과를 `/content/dataset_merged`에 저장하고 `dataset.location`이 이를 가리키도록 해서 기존 학습 코드(`train.ipynb`) 그대로 재사용 가능
- Roboflow "Generate Version" 시 **Resize를 640×640(학습 imgsz와 일치)**, **"Fit" 방식(Stretch 아닌)** 권장 — 비율 왜곡 시 도형(d6/d8/d12/d20) 인식 정확도 저하 우려
- Version 2에 과일 포함 확인, 병합 결과 9클래스(apple/banana/d12/d20/d6/d8/orange/pineapple/flag) 정상 확인됨. **재학습은 아직 진행 전** — 다음 단계

### 그 외 팀원 작업 (병렬로 반영됨)

- 그립 확인(`GRIP_CHECK`) 기능을 넣었다가 다시 뺌, 대신 정렬 재확인 유지 + 집기 후 후진 동작 추가
- `wheels_motion_test.py`, `motion_test.py`(YOLO 없이 프레임 차이로 이동/정지 판별) 신규 테스트 스크립트
- `arm_angle_test`에 카메라 스트림·사진 촬영·그리퍼 열기/닫기 기능 추가, 팔 임의 각도 이동 명령(`arm_to`) 추가
- `ARM_SPEED` 160→120→80(감속), `armDown`/`armUp` wait_ms도 비례 조정 후 팀원이 다시 단축(200/1500) — 실측 아닌 요청 기반 조정이라 실물 확인 필요 남아있음

### 다음 세션 우선순위

1. **배터리 스왑 테스트** — 원인 확정의 핵심, 아직 미검증
2. 병합 데이터셋으로 **재학습 실행** (Colab, `train.ipynb`) → `best.pt` 교체 → `trt_export.py` 재변환 → `best.engine` 갱신
3. GO_TO_STORAGE 밀집 이동 탐색 로직 실전 테스트 (필드에서 실제로 태극기 발견율이 올라가는지 확인)
4. end-to-end 통합 테스트 (탐지 → grip → GO_TO_STORAGE → 진입까지 전체 사이클)
5. 카메라 hang 빈도 자체를 줄일 방법 있는지 검토(USB 대역폭 이슈 근본 원인은 아직 미해결, 재연결 체계로 완화만 한 상태)
