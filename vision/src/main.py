...
MODEL_PATH = os.path.join(BASE_DIR, "model", "best_final.engine")  # 2026-07-24 test-best-final 브랜치 — TensorRT 변환 완료
...
FLAG_FINAL_FORWARD_SECS  = 0.15   # 도달 판정 후 직진하는 시간
...
class RobotState(Enum):
    ...
    GO_TO_STORAGE  = "GO_TO_STORAGE"  # 밀집 방향 탐색 → 태극기 감지 시 잠깐 정지 → 좌우 정렬 → 정렬 끝나면 직진 접근 → area 임계 도달 시 정지(경기 종료 취급)
...
def _read_openrb_loop():
    global openrb_gripped, openrb_grip_failed, grip_success_count
    while True:
    ...
    while True:
                ...
                if storage_phase == 0:
                    ...
                    if not flag_candidates:
                        ...
                        if flag_miss_count <= FLAG_MISS_GRACE_FRAMES:
                            control_wheels(None)
                            print(f"[상태] 태극기 순간 놓침 ({flag_miss_count}/{FLAG_MISS_GRACE_FRAMES}) — 정지 대기", end="\r")
                        else:
                            control_wheels(None)
                            storage_phase     = 0
                            flag_miss_count   = 0
        ...
        if _watching or (RECORD and not RECORD_RAW) or not HEADLESS:
            ...
            cv2.putText(annotated_frame, f"PICKED: {grip_success_count}",
                        (w - 150, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
